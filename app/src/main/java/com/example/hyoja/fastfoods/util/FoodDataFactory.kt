...
import com.example.hyoja.fastfoods.model.NewMenu5
import com.example.hyoja.fastfoods.model.NewMenu6
...
class FoodDataFactory {
    ...
    private fun fillCategory(){
        val value: FoodUtilValue = FoodUtilValue()

        if(newMenuArrayList.size<value.newMenuListSize){
            while (foodArrayList.size<=value.newMenuListSize){
    ...
    private fun makeFoodList(){
        ...
        foodArrayList.add(NewMenu5())
        foodArrayList.add(NewMenu6())
